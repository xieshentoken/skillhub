...
python3 -m skillhub link <id> --agents pi,codex [--force] [--dry-run] [--skip-conflicts] [--json]
...
python3 -m skillhub backups [--json] / rollback <ts>        # 备份与回滚（硬链接去重）
...
python3 tests/test_projection.py              # 17 项集成测试（用临时目录，不碰真实环境）
```
...
4. **投影前先备份**：`~/.skillhub/backups/<ts>/` 保存 store + index + 被替换的冲突目录，支持 `rollback`；备份文件对 store 用硬链接去重（store 视为不可变），自动只保留最近 `SKILLHUB_MAX_BACKUPS` 份。
...
7. **风险门禁**：高危 skill（`SKILLHUB_RISK_GATE`，默认 `sudo`）link 默认拦截，需 `--force`。
